...
DROP FUNCTION IF EXISTS check_burn_streak_reset();
DROP FUNCTION IF EXISTS is_edt_midnight();
DROP FUNCTION IF EXISTS reset_burn_streaks();

-- Create function to handle immediate boost completion with simplified streak logic
...
BEGIN
    ...
    IF NOT v_had_boost_yesterday THEN
        -- Reset streak if no boosts yesterday
        v_burn_streak := 1;
    ELSE
        -- Increment streak if had boosts yesterday
