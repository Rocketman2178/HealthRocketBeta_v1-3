-- Create function to test challenge players query with full details
...
AS $$
    -- Get all users with active challenges and their community info
    SELECT 
        c.user_id,
        u.name,
        u.email,
...
BEGIN
    ...
    IF v_count = 0 THEN
        RAISE NOTICE E'\nNo players found for challenge tc0';
    ELSE
        RAISE NOTICE E'\nTotal players found: %', v_count;
    END IF;
END $$;