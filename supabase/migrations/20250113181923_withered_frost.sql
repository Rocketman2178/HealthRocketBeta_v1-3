...
-- Create function to test challenge players query with plan field
...
AS $$
    -- Get users with active challenges including plan field
    SELECT DISTINCT ON (c.user_id)
        c.user_id,
        u.name,
        u.avatar_url,
        u.level,
        COALESCE(u.health_score, 7.8) as health_score,
        COALESCE(u.healthspan_years, 0) as healthspan_years,
        COALESCE(u.plan, 'Pro Plan') as plan,  -- Default to Pro Plan if null
        u.created_at,
        u.burn_streak
    FROM challenges c
