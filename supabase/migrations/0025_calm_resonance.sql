-- Add initial_healthspan column to users table
ALTER TABLE public.users 
ADD COLUMN IF NOT EXISTS initial_healthspan integer;

-- Function to get initial healthspan
CREATE OR REPLACE FUNCTION get_initial_healthspan(p_user_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- If not set, get from first health assessment
  IF v_initial_healthspan IS NULL THEN
    ...
    -- Store it in users table if found
  ...
  RETURN v_initial_healthspan;
END;
$$;

-- Update health assessment function to calculate healthspan years correctly
